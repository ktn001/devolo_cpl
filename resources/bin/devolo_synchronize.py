    ...
    try:
        for i in range(20):
            if not hasattr(device.plcnet,'async_get_network_overview'):
                logging.info(f"{serial}: pas d'attribut 'async_get_network_oveview' ==> reconnection {i+1}")
        ...
        for i in range (0, len(network.devices)):
            ...
            if dev.mac_address in names:
                if names[dev.mac_address] != dev.user_device_name:
                    warning(f"Deux noms différents pour la mac address {dev.mac_address}: {names[dev.mac_address]} et {dev.user_device_name}")
            else:
