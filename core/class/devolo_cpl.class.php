...
class devolo_cpl extends eqLogic {
	...
	*/
			...
			if (preg_match('/([^\s]+)[\s]*([>=~]=)[\s]*([\d+\.?]+)$/', $line, $need) === 1) {
				if (preg_match('/' . $need[1] . '==([\d+\.?]+)/', $packages, $install) === 1) {
					if ($need[2] == '==' && $need[3] != $install[1]) {
						log::add(__CLASS__,'debug',sprintf(__("Module python %s : Mauvaise version installée ",__FILE__),$need[1]));
						return false;
					} elseif (version_compare($need[3], $install[1], '>')) {
						log::add(__CLASS__,'debug',sprintf(__("Module python %s : version installée trop ancienne!",__FILE__),$need[1]));
						return false;
					}
				} else {
					log::add(__CLASS__,"debug",sprintf(__("Module python %s introuvable!",__FILE__),$line));
					return false;
				}
			}
		}
	...
	public static function daemon_stop() {
		...
		sleep(2);
		system::kill('python.*devolo_cpld.py');
		sleep(1);
	}
	...
	public static function syncDevolo() {
		$path = realpath(dirname(__FILE__) . '/../../resources/bin');
		$cmd = self::PYTHON_PATH . " " . $path . '/devolo_synchronize.py';
		$loglevel = log::convertLogLevel(log::getLogLevel(__CLASS__));
